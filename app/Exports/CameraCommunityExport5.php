...
class CameraCommunityExport5 implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, 
...
class SummarySheet implements FromCollection, WithTitle, ShouldAutoSize, WithStyles
{
    protected $request;
    ...
    public function collection()
    {
        ...
        if($this->request->sub_region) {
            ...
            $totalCommunitiesQuery->where('communities.sub_region_id', $this->request->sub_region);
            $totalRepositoriesQuery->where('repositories.sub_region_id', $this->request->sub_region);
        }
        ...
        // Labels in the desired order (these will go into column A)
        $labels = [
        ...
        // Corresponding keys for values (these will go into column B)
        $keys = [
            'current_installed_cameras_community',
            'current_installed_cameras_warehouse',
            'total_cameras_installed_ever_community',
            'total_cameras_installed_ever_warehouse',
            'demolition_cameras',
            'settler_vandalism_cameras',
            'stolen_cameras',
            'damaged_cameras',
            'total_replacement_incident_cameras',
            'total_installations',
            'total_initial_cameras',
            'total_added_cameras',
            'total_replaced_cameras',
            'total_damaged_cameras',
            'total_compounds',
            'total_beneficiaries',
            'total_households'
        ];
...
class InstalledCamerasSheet implements FromCollection, WithHeadings, WithTitle, ShouldAutoSize, WithStyles
{
    ...
    public function headings(): array
    {
        return [
            ...
            "Children", "Donors", "Cameras incidents", "Notes"
        ];
    }
