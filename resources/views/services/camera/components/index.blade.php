...
<br>
<h4 class="py-3 breadcrumb-wrapper mb-4">
All<span class="text-muted fw-light"> Camera Components & Accessories</span> 
</h4>

@if(session()->has('component_accessory_message'))
    <div class="row">
        <div class="alert alert-success">
            {{ session()->get('component_accessory_message') }}
        </div>
    </div>
@endif 
            ...
            @if(Auth::guard('user')->user()->user_type_id != 1 ||
                ...
                <div>
                    <p class="card-text">
                        <div>
                            <button type="button" class="btn btn-success" 
                                data-bs-toggle="modal" data-bs-target="#createComponentAccessory">
                                Create New One	
                            </button>
                        </div>
                        @include('services.camera.components.component_accessory.create')
                    </p>
                </div>
            @endif
            <table id="componentAccessoryTable" 
                class="table table-striped data-table-component-accessory my-2">
                <thead>
                    <tr>
...
<script type="text/javascript">
    $(function () {
        ...
        var table3 = $('.data-table-component-accessory').DataTable({
            processing: true,
            serverSide: true, 
            ajax: {
                url: "{{ route('camera-component-accessory.index') }}",
                data: function (d) {
                    d.search = $('input[type="search"]').val()
                }
            },
        ...
        // View edit page for component accessories
        $('#componentAccessoryTable').on('click', '.updateComponentAccessory',function() {
            var id = $(this).data('id');
            var url = window.location.href; 
            var newUrl = url.replace('camera-component', 'camera-component-accessory');
            newUrl = newUrl +'/'+ id +'/edit';
            // AJAX request
            $.ajax({
                url: 'camera-component-accessory/' + id + '/editpage',
                type: 'get',
                dataType: 'json',
                success: function(response) {
                    window.open(newUrl); 
        ...
        // Delete record for component accessories
        $('#componentAccessoryTable').on('click', '.deleteComponentAccessory', function() {
            var id = $(this).data('id');

            Swal.fire({
                icon: 'warning',
                title: 'Are you sure you want to delete this Camera Component & Accessory?',
                showDenyButton: true,
                confirmButtonText: 'Confirm'
            }).then((result) => {
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('deleteComponentAccessory') }}",
                        type: 'get',
                        data: {id: id},
                        success: function(response) {
                            if(response.success == 1) {
                                Swal.fire({
                                    ...
                                    $('#componentAccessoryTable').DataTable().draw();
                                });
                            } else {
