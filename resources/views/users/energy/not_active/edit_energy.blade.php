        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{route('all-meter.update', $energyUser->id)}}"
             ...
             enctype="multipart/form-data" >
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-6">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select id="selectedCommunityFbs" class="selectpicker form-control" name="community_id" data-live-search="true">
                                ...
                                <option value="{{$energyUser->Community->id}}" selected>{{$energyUser->Community->english_name}}</option>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="card p-3 mb-3" style="background:#f8f9fa; border:1px solid #e0e0e0;">
                                            ...
                                            <div class="form-check mb-2">
                                                ...
                                                <input class="form-check-input" type="checkbox" value="1" id="isMeterSharedFbs" name="is_meter_shared" {{ old('is_meter_shared', isset($energyUser->is_meter_shared) ? $energyUser->is_meter_shared : 0) ? 'checked' : '' }}>
                ...
                @if(count($energyNewDonors) > 0)

                    <table id="energyNewDonorsTable" class="table table-striped data-table-energy-donors my-2">
                        
                        <tbody>
                            @foreach($energyNewDonors as $energyNewDonor)
                            <tr id="energyDonorRow">
                                <td class="text-center">
                                    {{$energyNewDonor->Donor->donor_name}}
                                </td>
                                <td class="text-center">
                                    <a class="btn deleteEnergyNewDonor" id="deleteEnergyNewDonor" data-id="{{$energyNewDonor->id}}">
                                        <i class="fa fa-trash text-danger"></i>
                                    </a>
                                </td>
                            </tr>
                        ...
                        <div class="col-xl-4 col-lg-4 col-md-4">
                            <fieldset class="form-group">
                                ...
                                <select class="selectpicker form-control" 
                                    ...
                                    <option selected disabled>Choose one...</option>
                                    @foreach($moreNewDonors as $moreNewDonor)
                                        <option value="{{$moreNewDonor->id}}">
                                            {{$moreNewDonor->donor_name}}
                                        </option>
                                    @endforeach
                                </select>
                            </fieldset>
...
<script type="text/javascript">
    ...
    $('#energyNewDonorsTable').on('click', '.deleteEnergyNewDonor',function() {
        ...
        Swal.fire({
            ...
            if(result.isConfirmed) {
                $.ajax({
                    url: "{{ route('deleteEnergyNewDonor') }}",
                    type: 'get',
                    data: {id: id},
                    success: function(response) {
                        if(response.success == 1) {
...
<script>
    ...
    (function(){
        ...
        var initialShared = @json($sharedHouseholdIds ?? []);

        var initialSharedMap = @json($sharedHouseholdMap ?? []);
